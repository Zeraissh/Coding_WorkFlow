...
def mock_pty(monkeypatch):
    ...
    # Create bidirectional socket pairs to simulate PTY behavior
    ...
    r1, w1 = s1_a.fileno(), s1_b.fileno()
    r2, w2 = s2_a.fileno(), s2_b.fileno()
    ...
    def fake_write(fd, data):
        if fd in fd_to_sock:
            try:
                return fd_to_sock[fd].send(data)
            except OSError:
                return len(data)
    ...
    def fake_create_unix_pair_with_pty():
        ...
        port1 = f"/dev/pts/{10 + (call_count[0] - 1) * 2}"
        port2 = f"/dev/pts/{11 + (call_count[0] - 1) * 2}"
        return port1, port2
