        ...
        try {
            ...
            if (!budgetCheck.canContinue) {
                ...
                if (budgetCheck.warning)
                    result.error = budgetCheck.warning;
                return result;
            }
            const response = await askLLM(finalSystemPrompt, [{ role: 'user', content: "Execute the sub-task." }], anthropicTools, async (name, input) => {
                toolCallCount++;
                ...
                if (task.id)
                    workflowEvents.emit('log', { taskId: task.id, message: `[${this.agentId}] [Tool Call] ${name}` });
