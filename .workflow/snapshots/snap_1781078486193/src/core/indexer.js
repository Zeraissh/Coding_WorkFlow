...
export class ProjectIndexer {
    ...
    metadataPath;
    initialized = false;
    constructor(cwd = process.cwd(), dim = 384) {
        this.cwd = cwd;
        this.dim = dim;
    ...
    async init() {
        ...
        if (fs.existsSync(this.indexPath) && fs.existsSync(this.metadataPath)) {
            try {
                ...
                this.chunksMap = new Map(metadata.chunks.map((c) => [c.id, c.chunk]));
    ...
    async search(query, topK = 3) {
        ...
        return result.neighbors.map((idx) => this.chunksMap.get(idx));
    }
    walkDir(dir, exts) {
        let results = [];
        let list = [];
        ...
        for (const file of list) {
            if (['node_modules', '.git', 'dist', 'build', '.workflow'].includes(file))
                continue;
