...
export class TokenBudgetManager {
    ...
    budget = null;
    allocations = new Map(); // subtaskId → allocation
    agentTaskMap = new Map(); // agentId → subtaskId
    listeners = [];
    agentCallCounts = new Map();
    /** 已完成任务集合（用于报表状态判定） */
    completedTasks = new Set();
    /** 当 budget 未初始化时的后备消耗追踪 */
    agentFallbackSpent = new Map();
    constructor() { }
    static getInstance() {
    ...
    static resetInstance() {
        TokenBudgetManager.instance = undefined;
    }
    configure(config) {
    ...
    allocateForTasks(subtasks) {
        ...
        // 计算总复杂度（默认值 5）
        const totalComplexity = subtasks.reduce((sum, t) => sum + (t.estimatedComplexity && t.estimatedComplexity > 0 ? t.estimatedComplexity : 5), 0);
        if (totalComplexity <= 0) {
            // 理论上不会发生，但做保护
            return [];
        }
        // 按权重分配
        const allocations = subtasks.map((task, index) => {
     ...
     */
    reportUsage(agentId, tokensUsed) {
        ...
        // 更新计数器
        this.budget.spent += tokensUsed;
        this.budget.taskSpent.set(subtaskId, (this.budget.taskSpent.get(subtaskId) || 0) + tokensUsed);
        this.budget.agentSpent.set(agentId, (this.budget.agentSpent.get(agentId) || 0) + tokensUsed);
     ...
     */
    rebalance(completedAgentId) {
        if (!this.config.enabled || !this.config.autoRebalance || !this.budget)
            return;
        ...
        // 剩余太少不值得重分配
        if (surplus < MIN_SURPLUS_FOR_REBALANCE)
            return;
        // 找出还活跃的其他子任务
     ...
     */
    requestExtraBudget(subtaskId, amount) {
        if (!this.budget || this.budget.freePool <= 0)
            return 0;
        const allocation = this.allocations.get(subtaskId);
        if (!allocation)
            return 0;
    ...
    getReport() {
        ...
        const tasksBreakdown = Array.from(this.allocations.entries()).map(([subtaskId, allocation]) => {
            const spent = this.budget.taskSpent.get(subtaskId) || 0;
            const percentUsed = allocation.allocatedTokens > 0
                ? spent / allocation.allocatedTokens
                : 0;
...
// 导出单例获取函数
export const tokenBudget = () => TokenBudgetManager.getInstance();
