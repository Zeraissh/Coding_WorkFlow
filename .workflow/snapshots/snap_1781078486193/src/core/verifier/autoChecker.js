...
export class AutoChecker {
    ...
    async check(agentLogs) {
        ...
        if (lintResult) {
            ...
            if (this.config.autoFix && lintErrors.length > 0) {
                autoFixSuggestions.push(`发现 ${lintErrors.length} 个 lint 错误，建议运行 npx eslint --fix 自动修复`);
            }
        }
    ...
    detectFileConflicts(agentLogs) {
        const fileWriters = new Map(); // file → Set<agentId>
     ...
     */
    isFileCreatedByAgents(agentLogs, importerFile, importPath) {
    ...
    async runLintCheck() {
        try {
            ...
            const result = await this.deps.runShell('npx eslint --format json --ext .ts,.tsx,.js,.jsx .', this.deps.cwd);
    ...
    parseImportsExports(content) {
        ...
        const importDeclRegex = /import\s+(?:type\s+)?(?:(?:\{([^}]*)\}|(\*\s+as\s+\w+)|(\w+))\s*,?\s*)?(?:,\s*(?:\{([^}]*)\}|(\w+)))?\s*from\s*['"]([^'"]+)['"]/g;
        let match;
    ...
    parseTscOutput(raw) {
        ...
        while ((match = regex2.exec(raw)) !== null) {
            if (match[1] && match[2] && match[4] && match[5]) {
                ...
                // 避免重复（同一错误可能被两个正则匹配）
                const isDuplicate = errors.some((e) => e.file === file &&
                    e.line === line &&
                    e.code === code);
                if (!isDuplicate) {
                    errors.push({
        ...
        try {
            ...
            return {
                ...
                duration: parsed.testResults
                    ? parsed.testResults.reduce((sum, r) => {
            ...
            return {
                passed: passedMatch && passedMatch[1] ? parseInt(passedMatch[1], 10) : 0,
                failed: failedMatch && failedMatch[1] ? parseInt(failedMatch[1], 10) : 0,
                output: raw,
            };
        }
    }
