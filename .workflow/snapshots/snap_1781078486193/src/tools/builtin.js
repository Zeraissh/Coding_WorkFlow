...
export function safeListDir(dir, maxDepth, currentDepth = 1) {
    if (currentDepth > maxDepth)
        return [];
    const entries = fs.readdirSync(dir, { withFileTypes: true });
    const results = [];
    for (const entry of entries) {
        if (['node_modules', 'dist', 'build', 'out', 'coverage', 'venv', '.venv', '__pycache__', '.next', '.cache'].includes(entry.name) || entry.name.startsWith('.'))
            continue;
...
function nativeGrep(dir, pattern) {
    const results = [];
    let regex;
    try {
    ...
    function walk(currentDir) {
        ...
        for (const entry of entries) {
            if (['node_modules', '.git', 'dist', 'build', 'out', 'coverage', 'venv', '.venv', '__pycache__', '.next', '.cache', '.workflow'].includes(entry.name))
                continue;
...
export async function executeBuiltinTool(name, args, agentId) {
    try {
        switch (name) {
            ...
            case 'read_file': {
                if (agentId)
                    await fslock().acquireRead(args.path, agentId);
            ...
            case 'semantic_code_search': {
                ...
                if (results.length === 0)
                    return 'No semantically related code found.';
