...
export class SemanticReviewer {
     ...
     * @returns 语义问题列表
     */
    async review(agentLogs, autoCheckResult) {
    ...
    buildReviewPrompt(diffs, autoCheckResult) {
        const filesSection = diffs
            .map((d) => `### ${d.file} (由 ${d.agentId} 生成)\n\`\`\`\n${d.content}\n\`\`\``)
            .join('\n\n');
...
`
            : '';
        const dimensionsSection = REVIEW_DIMENSIONS.map((d) => `### ${d.name}\n${d.prompt}`).join('\n\n');
    ...
    parseReviewResponse(raw) {
        ...
        try {
            ...
            return parsed
                ...
                .map((item) => ({
                severity: (['critical', 'warning', 'style'].includes(item.severity) ? item.severity : 'warning'),
                file: item.file,
                line: typeof item.line === 'number' ? item.line : undefined,
                category: (['logic', 'error_handling', 'redundancy', 'security', 'performance', 'style'].includes(item.category) ? item.category : 'logic'),
 ...
 * 生成人类可读的验证汇总
 */
export function generateSummary(autoCheck, semanticIssues, durationMs) {
    const lines = [];
    lines.push('## 验证报告\n');
...
//# sourceMappingURL=semanticReviewer.js.map