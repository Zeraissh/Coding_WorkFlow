...
export class FileLockManager {
     ...
     */
    async acquireWrite(filePath, agentId) {
     ...
     * 读锁不互斥，但记录访问者供冲突检测使用。
     */
    async acquireRead(filePath, agentId) {
        if (!this.config.enabled)
            return;
        const normalizedPath = this.normalizePath(filePath);
        const existing = this.locks.get(normalizedPath);
     ...
     */
    writeFile(filePath, agentId, content) {
        const normalizedPath = this.normalizePath(filePath);
        if (this.config.enabled) {
            const existing = this.locks.get(normalizedPath);
            if (!existing || existing.ownerAgentId !== agentId || existing.mode !== 'write') {
    ...
    grantWriteLock(normalizedPath, agentId) {
        this.locks.set(normalizedPath, {
            path: normalizedPath,
            ownerAgentId: agentId,
            mode: 'write',
        ...
        const timer = setTimeout(() => {
            ...
            if (lock && lock.ownerAgentId === agentId) {
                console.warn(`[FSLock] Write lock timeout for "${normalizedPath}" (agent: ${agentId}). Auto-releasing.`);
                this.release(normalizedPath, agentId);
            }
        }, this.config.timeoutMs);
        this.timeoutTimers.set(normalizedPath, timer);
    }
    processQueue(normalizedPath) {
        const queue = this.writeQueues.get(normalizedPath);
...
// 导出单例获取函数（便捷用法）
export const fslock = () => FileLockManager.getInstance();
