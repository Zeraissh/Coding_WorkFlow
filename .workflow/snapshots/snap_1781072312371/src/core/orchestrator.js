...
export class Orchestrator {
    ...
    constructor() {
        ...
        this.decomposer = new Decomposer({
            callLLM: async (prompt, opts) => {
                const response = await askLLM(prompt, [{ role: 'user', content: prompt }], undefined, undefined, opts?.temperature ?? 0.3, 'orchestrator');
    ...
    async planWorkflow(goal) {
        ...
        try {
            ...
            const tasks = decomposition.subtasks.map((t) => ({
                id: t.id,
            ...
            return {
                goal,
                tasks,
                parallelBatches: decomposition.parallelBatches.map((batch) => batch.map((t) => tasks.find((task) => task.id === t.id))),
...
Return ONLY valid JSON.`;
    ...
    async executeWorkflow(goal, options) {
