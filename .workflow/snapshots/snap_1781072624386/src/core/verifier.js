    ...
    constructor() {
        ...
        this.semanticReviewer = new SemanticReviewer({
            callLLM: async (prompt, opts) => {
                const response = await askLLM(prompt, [{ role: 'user', content: prompt }], undefined, undefined, opts?.temperature ?? 0.1, 'verifier');
                const textBlock = response.content.find(block => block.type === 'text');
                return textBlock?.text || '';
            },
        }, { autoCheck: true, semanticReview: true, autoFix: false });
    }
    async verifyAndSynthesize(plan, results, agentLogs) {
