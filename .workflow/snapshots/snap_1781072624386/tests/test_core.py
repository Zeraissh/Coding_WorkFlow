...
class TestCreateVirtualSerialPairUnix:
    ...
    def test_data_forwarded_between_ports(self, mock_pty, monkeypatch):
        ...
        # Write to the slave FD (w1) and read from the other slave (w2)
        r1 = mock_pty["r1"]
        r2 = mock_pty["r2"]
        w1 = mock_pty["w1"]
        w2 = mock_pty["w2"]
        ...
        import select
        ready, _, _ = select.select([w2], [], [], 0.2)
        if ready:
            data = os.read(w2, 1024)
            assert data == test_data
        else:
            assert False, "Timeout waiting for data on w2"

        os.write(w2, b"Forward me!")

        time.sleep(0.3)

        ready, _, _ = select.select([w1], [], [], 0.2)
        if ready:
            data = os.read(w1, 1024)
            assert data == b"Forward me!"
        else:
            assert False, "Timeout waiting for data on w1"
...
class TestCom0comHelpers:
    ...
    def test_find_setupc_finds_on_path(self, monkeypatch):
        ...
        monkeypatch.setattr(vs.os.path, "isfile", lambda x: True)
